...
const ApprovalSettings = () => {
 ...
 const fetchBudgetUsers = async () => {
  ...
  if (!error && data) {
    // Ensure always 4 rows (fill empty slots if needed)
    let filledData = [...data];
    while (filledData.length < 4) {
      filledData.push({ id: null, create_budget: false, view_budget: false, regular: false, pre_regular: false });
    }
    setBudgetUsers(filledData);
  } else if (error) {
  ...
  const handleBudgetPermissionToggle = async (userId, field, currentValue) => {
    try {
      ...
      if (!userId) {
        ...
        setBudgetUsers([data]);
        Swal.fire("Success", "Budget record created", "success");
        return;
      }
      ...
      setBudgetUsers((prev) =>
        prev.map((u) =>
          u.id === userId ? { ...u, [field]: updatedValue } : u
        )
      );
  ...
  return (
    <div style={containerStyle}>
      ...
      <h2 style={{ marginTop: '40px' }}>Budget Permissions</h2>

      {loadingBudget ? (
        <p>Loading budget permissions...</p>
      ) : (
        <table style={tableStyle}>
          <thead>
            <tr>
              <th style={thStyle}>User ID</th>
              <th style={thStyle}>Create Budget</th>
              <th style={thStyle}>View Budget</th>
              <th style={thStyle}>Regular</th>
              <th style={thStyle}>Pre-Regular</th>
            </tr>
          </thead>
          <tbody>
            {budgetUsers.map((user, idx) => (
              <tr key={idx}>
                <td style={tdStyle}>{user.id ?? "N/A"}</td>
                <td style={tdStyle}>
                  <input
                    type="checkbox"
                    checked={user.create_budget || false}
                    disabled={!user.id}
                    onChange={() =>
                      handleBudgetPermissionToggle(
                        user.id,
                        "create_budget",
                        user.create_budget
                      )
                ...
                <td style={tdStyle}>
                  <input
                    type="checkbox"
                    checked={user.view_budget || false}
                    disabled={!user.id}
                    onChange={() =>
                      handleBudgetPermissionToggle(
                        user.id,
                ...
                <td style={tdStyle}>
                  <input
                    type="checkbox"
                    checked={user.pre_regular || false}
                    disabled={!user.id}
                    onChange={() =>
                      handleBudgetPermissionToggle(
                        user.id,
