...
const GLOBAL_CSS = `
  ...
  .dist-wrap .btn-primary:hover   { background: #1d4ed8; box-shadow: 0 4px 14px rgba(37,99,235,.38); }
...
const Distributor = () => {
  ...
  const [form, setForm] = useState({ id: null, code: '', name: '', description: '', agent_code: '', mother_accounts_code: '', slp: '' });
  ...
  const handleDragEnd   = () => {
    ...
    const reordered = [...dupRows];
    const [moved] = reordered.splice(dragIdx.current, 1);
    reordered.splice(dragOver.current, 0, moved);
    setDupRows(reordered);
    dragIdx.current = dragOver.current = null;
  };
  ...
  const openAddModal = async () => {
    ...
    setForm({ id: null, code: nextCode, name: '', description: '', agent_code: '', mother_accounts_code: '', slp: '' });
    setIsEditing(false); setIsViewing(false);
    setSelectedMotherAccounts([]); setSelectedAgentUser(null); setSelectedAgentUsers([]);
    setModalOpen(true);
  };

  const populateFormFromDistributor = (d) => {
    setForm({ id: d.id, code: d.code, name: d.name || '', description: d.description || '', agent_code: d.agent_code || '', mother_accounts_code: d.mother_accounts_code || '', slp: d.slp != null ? String(d.slp) : '' });
  ...
  const openEditModal = (d) => { populateFormFromDistributor(d); setIsEditing(true); setIsViewing(false); setModalOpen(true); };
  const openViewModal = (d) => { populateFormFromDistributor(d); setIsEditing(false); setIsViewing(true); setModalOpen(true); };
  ...
  const handleMotherAccountToggle = (id) => {
    if (isViewing) return;
    const next = selectedMotherAccounts.includes(id) ? selectedMotherAccounts.filter(s => s !== id) : [...selectedMotherAccounts, id];
  ...
  const handleSubmit = async (e) => {
    ...
    const motherCodes = motherAccounts.filter(ma => selectedMotherAccounts.includes(ma.id)).map(ma => ma.code).join(',') || null;
    const slpVal = form.slp !== '' && form.slp != null ? parseInt(form.slp, 10) : null;
    try {
      if (isEditing && form.id) {
        const { error } = await supabase.from('distributors').update({ name: form.name.trim(), description: form.description?.trim() || null, agent_code: selectedAgentUsers.length > 0 ? selectedAgentUsers.map(a => a.UserID).join(',') : null, mother_accounts_code: motherCodes, slp: slpVal }).eq('id', form.id);
        ...
        const { error } = await supabase.from('distributors').insert([{ code, name: form.name.trim(), description: form.description?.trim() || null, agent_code: form.agent_code?.trim() || null, mother_accounts_code: motherCodes, slp: slpVal }]);
        if (error) throw error;
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({ title: 'Delete?', text: 'This cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc2626', confirmButtonText: 'Yes, delete' });
  ...
  /* ─── Export ─────────────────────────────────────────────────────── */
  const handleExport = () => {
    const headers = ['ID', 'Code', 'SLP', 'Distributor Name', 'Description', 'Agent Name', 'Agent Code', 'Mother Accounts Code'];
    const rows = distributors.map(d => {
      let agentNames = '';
      if (d.agent_code) {
        agentNames = String(d.agent_code).split(',').map(c => c.trim()).filter(Boolean).map(code => {
          const f = accountUsers.find(u => String(u.UserID) === code);
          return f ? f.name : code;
        }).join(', ');
      }
      return [d.id, d.code, d.slp ?? '', d.name, d.description || '', agentNames, d.agent_code || '', d.mother_accounts_code || ''];
    });
  ...
  /* ─── Import ─────────────────────────────────────────────────────── */
  // Maps CSV row (keys already lowercased) to DB fields.
  // Supports our own export headers: "Distributor Name", "Agent Code", "Mother Accounts Code", etc.
  const normalizeRow = (row) => {
    const slpRaw = (row['slp'] ?? '').toString().trim();
    const resolvedName =
      row['distributor name']?.trim() ||
      row['name']?.trim() || '';
    const resolvedCode =
      row['code']?.trim() || null;
    const resolvedDesc =
      row['description']?.trim() || null;
    // "Agent Code" is col 7 in our export; skip "Agent Name" (col 6 — display only)
    const resolvedAgentCode =
      row['agent code']?.trim() ||
      row['agentcode']?.trim() ||
      row['agent_code']?.trim() || null;
    const resolvedMotherCode =
      row['mother accounts code']?.trim() ||
      row['mother accounts co']?.trim() ||
      row['mother accounts']?.trim() ||
      row['mother_accounts_code']?.trim() || null;
    return {
      code: resolvedCode,
      name: resolvedName,
      description: resolvedDesc,
      agent_code: resolvedAgentCode,
      mother_accounts_code: resolvedMotherCode,
  ...
  const handleImport = (e) => {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        const norm = results.data
          .map(row => {
            const n = {};
            for (const k in row) if (k?.trim()) n[k.toLowerCase().trim()] = row[k];
            return n;
          })
          // Accept rows with either "distributor name" (our export) or plain "name"
          .filter(r => r['distributor name']?.trim() || r['name']?.trim());
        if (!norm.length) { Swal.fire('Notice', 'No valid rows found in CSV. Make sure the file has a "Distributor Name" or "Name" column.', 'info'); return; }
    ...
    try {
      ...
      for (const dup of dupRows) {
        if (dup.action === 'update') {
          const { error } = await supabase.from('distributors').update({ name: dup.csvRow.name, description: dup.csvRow.description, agent_code: dup.csvRow.agent_code, mother_accounts_code: dup.csvRow.mother_accounts_code, slp: dup.csvRow.slp, ...(dup.csvRow.code ? { code: dup.csvRow.code } : {}) }).eq('id', dup.existingRow.id);
          if (error) throw error;
          updated++;
        } else skipped++;
      }
      ...
      Swal.fire({ title: 'Import Complete!', html: `<div style="text-align:left;font-size:14px;line-height:2.2">✅ <b>${inserted}</b> inserted<br/>🔄 <b>${updated}</b> updated<br/>⏭️ <b>${skipped}</b> skipped</div>`, icon: 'success' });
    } catch (err) { Swal.fire('Error', err.message, 'error'); }
  ...
  const autoSave = debounce(async (f) => {
    ...
    await supabase.from('distributors').update({ name: f.name.trim(), description: f.description?.trim() || null, agent_code: f.agent_code?.trim() || null, mother_accounts_code: f.mother_accounts_code || null, slp: slpVal }).eq('id', f.id);
    fetchDistributors();
  }, 1000);

  const filteredDist = distributors.filter(d => {
    const t = searchTerm.toLowerCase();
    return String(d.name || '').toLowerCase().includes(t) || String(d.code || '').toLowerCase().includes(t) ||
      String(d.description || '').toLowerCase().includes(t) || String(d.agent_code || '').toLowerCase().includes(t) ||
  ...
  /* ═══════════════════════════════════════════════════════════════════ */
  return (
    <div className="dist-wrap" style={{ padding: '28px 24px', maxWidth: 1500, margin: '0 auto', backgroundColor: '#f8fafc', minHeight: '100vh', borderRadius: 16 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 20, alignItems: 'center' }}>
        ...
        <label htmlFor="import-csv" className="btn btn-emerald" style={{ marginBottom: 0, cursor: 'pointer' }}>
          <svg width="14" height="14" fill="currentColor" viewBox="0 0 16 16"><path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"/><path d="M7.646 1.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1-.708.708L8.5 2.707V11.5a.5.5 0 0 1-1 0V2.707L5.354 4.854a.5.5 0 1 1-.708-.708l3-3z"/></svg>
          Import CSV
        </label>
        <input id="import-csv" type="file" accept=".csv" onChange={handleImport} style={{ display: 'none' }} />

        {/* Search */}
        <div style={{ position: 'relative', flex: 1, minWidth: 220 }}>
          <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }}><IcoSearch /></span>
          <input className="dist-input" style={{ paddingLeft: 36, marginBottom: 0 }} type="text" placeholder="Search name, code, SLP, agent…" value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }} />
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 13, color: '#64748b', fontWeight: 600 }}>Rows:</span>
          <select className="dist-input" style={{ width: 70, marginBottom: 0 }} value={itemsPerPage} onChange={e => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }}>
      ...
      <div style={{ borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,.07)', border: '1px solid #e2e8f0' }}>
        <div style={{ overflowX: 'auto' }} className="dist-scroll">
          <table>
            <thead>
              <tr>
                {['ID','Code','SLP','Distributor Name','Description','Agent Name','Agent Code','Mother Accounts','Actions'].map(h => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan="9" style={{ textAlign: 'center', padding: 32, color: '#94a3b8' }}>Loading…</td></tr>
              ) : currentItems.length === 0 ? (
                <tr><td colSpan="9" style={{ textAlign: 'center', padding: 32, color: '#94a3b8' }}>No distributors found</td></tr>
              ) : currentItems.map(d => {
                const agentNames = d.agent_code
                  ? String(d.agent_code).split(',').map(c => c.trim()).filter(Boolean).map(code => {
                      const f = accountUsers.find(u => String(u.UserID) === code);
                      return f ? f.name : code;
                    }).join(', ')
                  : '';
      ...
      {modalOpen && (
        <div className="dist-overlay" onClick={() => setModalOpen(false)}>
          <div className="dist-modal" style={{ maxWidth: 680 }} onClick={e => e.stopPropagation()}>
            {/* Modal header */}
            ...
            <div style={{ padding: '0 24px 24px' }}>
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <textarea className="dist-input" style={{ height: 64, resize: 'vertical' }} name="description" value={form.description} onChange={handleChange} disabled={isViewing} />
                </div>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button type="button" className="btn btn-primary btn-icon" onClick={() => { if (!isViewing) { fetchAccountUsers(); setAccountUserSearch(''); setUserModalOpen(true); } }} disabled={isViewing} title="Pick agent">
                      <IcoSearch />
                    </button>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    {!isViewing && (
                      <button type="button" className="btn btn-muted" style={{ padding: '4px 10px', fontSize: 12 }}
                        onClick={() => {
                          if (selectedMotherAccounts.length === motherAccounts.length) { setSelectedMotherAccounts([]); setForm(p => ({ ...p, mother_accounts_code: '' })); }
                          else { setSelectedMotherAccounts(motherAccounts.map(m => m.id)); setForm(p => ({ ...p, mother_accounts_code: motherAccounts.map(m => m.code).join(',') })); }
                        }}>
                        {selectedMotherAccounts.length === motherAccounts.length ? 'Uncheck All' : 'Check All'}
                      </button>
                    )}
                  </div>
                  <div className="dist-scroll" style={{ maxHeight: 160, overflowY: 'auto', border: '1.5px solid #e2e8f0', borderRadius: 10, padding: '8px 12px', background: '#fafafa' }}>
          ...
          <div className="dist-modal" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '16px 24px' }}>
              ...
              <div style={{ marginTop: 14, display: 'flex', justifyContent: 'flex-end' }}>
                <button className="btn btn-primary" onClick={() => setUserModalOpen(false)}>Done</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ══ Duplicate Review Modal (with Drag & Drop) ═════════════════════ */}
      {dupModalOpen && (
        <div className="dist-overlay" style={{ zIndex: 1100 }} onClick={() => !importProcessing && setDupModalOpen(false)}>
          <div className="dist-modal" style={{ maxWidth: 860 }} onClick={e => e.stopPropagation()}>

            {/* Header */}
            <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #fde68a', background: 'linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%)', borderRadius: '16px 16px 0 0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                <div>
                  <h3 style={{ margin: '0 0 4px', fontSize: 19, fontWeight: 700, color: '#92400e' }}>
            ...
            <div style={{ padding: '16px 24px' }}>
              ...
              <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                <IcoDrag /> Drag rows by the handle to reprioritize order before confirming
              </div>
                ...
                <table style={{ fontSize: '13px' }}>
                  <thead>
                    <tr>
                      ...
                      <th style={{ background: '#1a57ff' }}>📥 CSV (Incoming)</th>
                  ...
                  <tbody>
                    {dupRows.map((dup, idx) => (
                      <tr key={idx}
                        ...
                        style={{ background: idx % 2 === 0 ? '#fff' : '#fafafa', verticalAlign: 'top', cursor: 'default' }}
                      >
                        {/* Drag handle */}
                        <td style={{ textAlign: 'center', padding: '12px 4px' }}>
                          <span className="drag-handle"><IcoDrag /></span>
                        </td>
                        ...
                        <td style={{ padding: '12px 14px' }}>
                          ...
                          {dup.csvRow.description && <div style={{ color: '#94a3b8', fontSize: 12, marginTop: 3, fontStyle: 'italic' }}>{dup.csvRow.description.length > 52 ? dup.csvRow.description.substring(0, 52) + '…' : dup.csvRow.description}</div>}
                        </td>
                        ...
                        <td style={{ padding: '12px 14px' }}>
                          ...
                          {dup.existingRow.description && <div style={{ color: '#94a3b8', fontSize: 12, marginTop: 3, fontStyle: 'italic' }}>{dup.existingRow.description.length > 52 ? dup.existingRow.description.substring(0, 52) + '…' : dup.existingRow.description}</div>}
                        </td>
                        ...
                        <td style={{ padding: '12px 14px', textAlign: 'center' }}>
                          <div className="action-pill">
                            <button
                              className={dup.action === 'update' ? 'active-update' : ''}
                              onClick={() => handleDupActionChange(idx, 'update')}
                            >🔄 Update</button>
                            <button
                              className={dup.action === 'skip' ? 'active-skip' : ''}
                              onClick={() => handleDupActionChange(idx, 'skip')}
                            >⏭️ Skip</button>
                          </div>
                        </td>
                      </tr>
              ...
              <div style={{ marginTop: 16, padding: '14px 18px', background: '#f8fafc', borderRadius: 12, border: '1.5px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <button className="btn btn-primary"
                    onClick={handleConfirmImport} disabled={importProcessing}
