...
const RegularVisaForm = () => {
      ...
      try {
        ...
        for (const record of approvalData) {
          if (!latestResponseMap.has(record.PwpCode)) {
            latestResponseMap.set(
              record.PwpCode,
              record.Response.toLowerCase()
            );
          }
        }
        ...
        mergedData.forEach((item) => {
          console.log(
            `${item.pwp_code} - ${item.remainingbalance} - ${item.createForm}`
          );
        });
  ...
  const calculateGrandTotals = () => {
    ...
    return allAccountCodes.reduce(
      (grandTotal, accountCode) => {
        ...
        Object.keys(accountTotals.UOMCount).forEach((uom) => {
          grandTotal.UOMCount[uom] =
            (grandTotal.UOMCount[uom] || 0) + accountTotals.UOMCount[uom];
        });
  ...
  const handleFormChange = async (e) => {
    ...
    if (name === "distributor" || name === "accountType") {
      setRowsAccounts([]);
      console.log(
        "🧹 Cleared rowsAccounts due to distributor/accountType change"
      );
    }
    ...
    if (name === "distributor") {
      try {
        ...
        while (hasMore) {
          ...
          console.log(
            `✅ Batch ${Math.floor(offset / batchSize) + 1} fetched: ${
              data?.length || 0
            } records`
          );
  ...
  const handleImportCSV = (file) => {
    ...
    reader.onload = (evt) => {
      ...
      if (headerRowIndex === -1) {
        const errMsg =
          "Ops! The imported file must have all required columns (ACCOUNT_CODE, ACCOUNT_NAME, BUDGET) in the same row.";
        Swal.fire({
          icon: "error",
          title: "Import Error",
          text: errMsg,
      ...
      const importedAccountCodes = importedRows
        .map((row) => row[headerRow.indexOf("ACCOUNT_CODE")] || "")
        .filter((code) => code !== "");
      ...
      const invalidCodes = importedAccountCodes.filter(
        (code) => !uiAccountCodes.includes(code)
      );
      ...
      setRowsAccounts((prevRows) => {
        ...
        newAccounts.forEach((newAccount) => {
          const existingIndex = updatedRows.findIndex(
            (r) => r.account_code === newAccount.account_code
          );
          if (existingIndex !== -1) {
  ...
  const handleExportCSV = () => {
    ...
    const selectedAccounts = accountTypes.filter((account) =>
      formData.accountType.includes(account.code)
    );
    ...
    const exportData = selectedAccounts.map((account) => {
      ...
      const existingRow = rowsAccounts.find(
        (r) => r.account_code === account.code
      );
      const budgetValue =
  ...
  const handleSubmitFormAndAttachments = async () => {
    try {
      ...
      if (distributorCode) {
        const { data: distributorsData, error: distributorError } =
          await supabase
            .from("distributors")
            .select("code")
          ...
          <div>
            <form onSubmit={submit_all}>
                ...
                {shouldShowCategory() && (
                  <div className="col-md-4" style={{ position: "relative" }}>
                    ...
                    <input
                      ...
                      style={{
                        borderColor:
                          formData.categoryName?.length > 0 ? "green" : "",
                        transition: "border-color 0.3s",
                        paddingRight: "35px",
                        cursor: "pointer",
                      }}
                          ...
                          >
                            {filteredList.length > 0 ? (
                              filteredList.map((cat) => {
                                const isChecked =
                                  formData.categoryCode?.includes(cat.code);
                ...
                <div className="col-md-4" style={{ position: "relative" }}>
                  ...
                  >
                        ...
                        {accountTypes
                          ...
                          .map((opt) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={formData.accountType.includes(
                                  opt.code
                                )}
                    ...
                    <Modal.Footer>
                      <Button
                        variant="light"
                        onClick={() => setShowModal_Account(false)}
                      >
                        Close
                      </Button>
                    </Modal.Footer>
              ...
              <div className="card card-3d mt-4">
                ...
                <div
                  className="toggle-group mb-3"
                  role="group"
                  aria-label="Cover PWP Toggle"
                >
                  <input
                    type="radio"
                    id="coverYes"
                  ...
                  />
                  <label htmlFor="coverYes" className="toggle-label">
                    YES
                  </label>
                  ...
                  />
                  <label htmlFor="coverNo" className="toggle-label">
                    NO
                  </label>
                </div>
                    ...
                    >
                      {coverPwpWithStatus
                        .filter(
                          (cp) =>
                            cp.pwp_code &&
                            typeof cp.pwp_code === "string" &&
                            cp.pwp_code
                              .toLowerCase()
                              .includes(coverPwpSearch.toLowerCase())
                        )
                        .filter((cp) => {
                          // Show if admin
                          ...
                          return (
                            cp.createForm?.toLowerCase().trim() ===
                            currentUserName
                          );
                        })
                        .map((cp, idx) => {
                            ...
                            >
                              ...
                              >
                                {cp.remainingbalance !== null
                                  ? cp.remainingbalance.toLocaleString(
                                      "en-US",
                                      {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2,
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (formData.activityName === "BAD ORDER") {
                      ...
                      setStep(4);
                      console.log(
                        "⛔ BAD ORDER selected → skipping SKU/accounts checks, going to Step 3"
                      );
                    } else if (setting?.sku) {
                      setStep(1);
            ...
            <Card border="primary" className="shadow">
              {formData.isPartOfCoverPwp &&
                ...
                selectedBalance !== null && (
                  ...
                  >
                    ...
                    <div
                      className="card border-success mb-3 shadow"
                      style={{ width: "22rem" }}
                    >
                      ...
                      <div className="card-body text-center">
                        {(() => {
                          ...
                          const selected = parseFloat(selectedBalance || 0);
                          const creditBudget = parseFloat(
                            formData?.amountbadget || 0
                          );
                          const netTotal =
                            grandTotals.BILLING_AMOUNT - grandTotals.DISCOUNT;
                          const remainingSkuBudget =
                            selected - netTotal - creditBudget;
                          return (
                            <>
                              <p
                                className="card-text"
              ...
              <Card.Body>
                ...
                {tabs.length > 0 && (
                  <Nav
                    variant="tabs"
                    activeKey={selectedAccountForSku}
                    className="mb-3"
                  >
                    {tabs.map((tab) => (
                      <Nav.Item key={tab}>
                        ...
                        >
                            ...
                            style={{
                              backgroundColor:
                                selectedAccountForSku === tab
                                  ...
                                  : "#8ea1b4ff",
                              color:
                                selectedAccountForSku === tab ? "#fff" : "#000",
                              borderRadius: "10px",
                              marginRight: "8px",
                              fontWeight: "500",
                          ...
                          >
                            {tab === "ALL_ACCOUNTS"
                              ? "🔍 All Accounts"
                              : accountTypes.find((acc) => acc.code === tab)
                                  ?.name || tab}
                          </Nav.Link>
                        </motion.div>
                      </Nav.Item>
                ...
                {selectedAccountForSku === "ALL_ACCOUNTS" ? (
                  ...
                  <div>
                    ...
                    {accountTypes
                      ...
                      .map((account) => {
                        ...
                        const accountTotals = calculateAccountSkuTotals(
                          account.code
                        );
                          ...
                          <div key={account.code} className="mb-4">
                            <div className="d-flex justify-content-between align-items-center mb-2">
                              ...
                              <div className="d-flex gap-2">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    addSkuRowForAccount(account.code)
                                  }
                                >
                                  Add SKU
                                </Button>
                            ...
                            {accountRows.length > 0 ? (
                                ...
                                >
                                      ...
                                      return (
                                        <tr key={`${account.code}-${idx}`}>
                                          ...
                                          >
                                            <Form.Control
                                              value={
                                                categoryListing.find(
                                                  (sku) =>
                                                    sku.sku_code === row.SKUITEM
                                                )
                                                  ? `${row.SKUITEM} - ${
                                                      categoryListing.find(
                                                        (sku) =>
                                  ...
                                  <tfoot className="table-primary text-white">
                                    <tr>
                                      ...
                                      <td>
                                        <strong>
                                          {accountTotals.SRP.toFixed(2)}
                                        </strong>
                                      </td>
                                      <td>
                                        <strong>{accountTotals.QTY}</strong>
                                      </td>
                                      <td></td>
                                      <td>
                                        <strong>
                                          {accountTotals.BILLING_AMOUNT.toFixed(
                                            2
                                          )}
                                        </strong>
                                      </td>
                                      <td>
                                        <strong>
                                          {accountTotals.DISCOUNT.toFixed(2)}
                                        </strong>
                                      </td>
                                      <td>
                                        <strong>
                                          {accountTotals.TOTAL_AMOUNT.toFixed(
                                            2
                                          )}
                                        </strong>
                                      </td>
                              ...
                              <div className="text-center p-3 bg-light rounded">
                                <small className="text-muted">
                                  No SKU entries for this account
                                </small>
                              </div>
                            )}
                            <hr />
                  ...
                  <div>
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      <h5>
                        SKU Listing for:{" "}
                        {accountTypes.find(
                          (acc) => acc.code === selectedAccountForSku
                        )?.name || selectedAccountForSku}
                      </h5>
                      <Button
                        variant="primary"
                        size="sm"
                        onClick={() =>
                          addSkuRowForAccount(selectedAccountForSku)
                        }
                      >
                        Add SKU Row
                      </Button>
                    ...
                    <div style={{ overflowX: "auto" }}>
                      ...
                      >
                        ...
                        <tbody>
                          {(accountSkuRows[selectedAccountForSku] || []).map(
                            (row, idx) => (
                        ...
                        {selectedAccountForSku &&
                          accountSkuRows[selectedAccountForSku]?.length > 0 && (
                            <tfoot>
                              <tr className="table-info">
                                <th>Account Total</th>
                                <th>
                                  {calculateAccountSkuTotals(
                                    selectedAccountForSku
                                  ).SRP.toFixed(2)}
                                </th>
                                <th>
                  ...
                  <div className="text-center p-4 bg-light rounded">
                    <p className="text-muted mb-0">
                      Please select an account to manage SKU listings
                    </p>
                  </div>
                )}
              </Card.Body>
                  ...
                  <div className="d-flex justify-content-between align-items-start gap-4">
                    ...
                    >
                      ...
                      <div className="card-body text-center">
                        ...
                        <small className="text-muted d-block">
                          Total from Accounts Table: ₱
                          {totalAllocatedFromAccounts.toLocaleString("en-PH", {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </small>
                        ...
                        {accountTypes
                          ...
                          .map((account) => {
                            ...
                            return (
                              <tr key={account.id}>
                                <td>
                                  <Form.Control value={account.code} disabled />
                                </td>
                                <td>
                                  <Form.Control value={account.name} disabled />
                                </td>
                                <td>
                                  <Form.Control
                                    type="number"
                                      ...
                                      setRowsAccounts((prevRows) => {
                                        ...
                                        if (existingIndex !== -1) {
                                          ...
                                          if (existingBudget !== newBudget) {
                                            ...
                                            console.log(
                                              `[${now}] 🔄 Budget updated for "${account.account_name}" (${account.code}): ₱${existingBudget} ➡️ ₱${newBudget}`
                                            );

                                            // Log total budget after update
                                            const totalBudget = updated
                                              .filter((row) =>
                                                formData.accountType.includes(
                                                  row.account_code
                                                )
                                              )
                                              .reduce(
                                          ...
                                          const now =
                                            new Date().toLocaleString();
                                          console.log(
                                            `[${now}] ➕ New account budget added: "${account.account_name}" (${account.code}) - ₱${newBudget}`
                                          );
