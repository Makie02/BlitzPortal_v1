...
function EditModal({ isOpen, onClose, rowData, filter = "all", }) {
    ...
    try {
      ...
      for (const account of accountData) {
        const { data: existingAccounts, error: selectAccountError } = await supabase
          ...
          .eq('regularcode', formData.regularpwpcode)
          .eq('id', account.id); // match by id only
        ...
        if (existingAccounts && existingAccounts.length > 0) {
          ...
          console.log(`No matching account found for id ${account.id}, skipping update.`);
        }
      }
  ...
  // totalBeforeDiscountAll and totalDiscountAll unchanged
  ...
  // **Current total billing sums total_amount now**
  ...
  const unifiedRemainingBalance = (() => {
    ...
    // If billing increases, remaining balance decreases
    // If billing decreases, remaining balance increases
    return currentRemainingBalance - billingChange;
  })();
  // Billing difference and adjusted remaining balance unchanged
  ...
  const adjustedRemainingBalance =
    billingDifference !== 0
      ? remainingBalanceOriginal - billingDifference
      : remainingBalanceOriginal;
  ...
  useEffect(() => {
    const fetchDistributors = async () => {
      const { data, error } = await supabase
        ...
        .eq("username", loggedInUsername); // 👈 filter by logged-in user
  ...
  // Fetch mother accounts on mount
  useEffect(() => {
  ...
  // Fetch sub-accounts for a mother
              ...
              if (type === "select" && (name === "distributor" || name === "distributor_code")) {
                return (
                  <div key={name} style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    <select
                      name={name}
                      value={value || ""} // store code in value
                    ...
                    >
                      ...
                      {/* Keep previous value if it's not in filteredDistributors */}
                                    ...
                                    <input
                                      ...
                                      onChange={() => {
                                        if (selectedMother.name === "NON-CHAIN") {
                                          ...
                                          setShowModalCategory(prev => ({ ...prev, [name]: false })); // auto-close modal
                                        }
                                      }}
                                      id={`sub_${s.id}`}
              ...
              >
                ...
                <tbody>
                  {skuList.map(({ id, account_name, sku_code, srp, qty, uom, discount, total_amount }) => {
                    ...
                    // Debugging console log (optional)
                    // console.log(`Row ${id}: Calculated Billing = ${(srpNum * qtyNum - discountNum).toFixed(2)}, total_amount = ${totalAmount.toFixed(2)}`);
                ...
                <tfoot>
                  <tr>
                    ...
                    <td style={{ textAlign: "right", padding: "12px", border: "1px solid #ddd" }}>
                      { /* Sum of total_amount now, matching displayed billing amounts */}
