...
function EnhancedDatabaseInterface() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const userIdToNameMap = new Map(users.map(u => [u.UserID, u.name.toLowerCase().trim()]));
...
const handleDelete = async (rowId) => {
  ...
  try {
    ...
    if (row.cover_code && row.cover_code.startsWith("CL")) {
      ...
      if (attachmentError)
        throw new Error(`Failed to delete claims attachments: ${attachmentError.message}`);

      // 2. Delete approved history (budget)
      const { error: approvedHistoryError } = await supabase
        .from("approved_history_budget")
        .delete()
        .eq("pwp_code", coverCode);
      if (approvedHistoryError)
        throw new Error(`Failed to delete approved history (claims): ${approvedHistoryError.message}`);
      ...
      if (approvalError)
        throw new Error(`Failed to delete approval history (claims): ${approvalError.message}`);

      // 4. Delete from amount_badget
      const { error: amountError } = await supabase
        .from("amount_badget")
        .delete()
        .ilike("pwp_code", coverCode);
      if (amountError)
        throw new Error(`Failed to delete amount budget (claims): ${amountError.message}`);

      // 5. Delete from Claims_AccountBudgetTable
      const { error: accountBudgetError } = await supabase
        .from("Claims_AccountBudgetTable")
        .delete()
        .eq("code_pwp", coverCode);
      if (accountBudgetError)
        throw new Error(`Failed to delete Claims_AccountBudgetTable records: ${accountBudgetError.message}`);

      // 6. Delete from Claims_Badorder
      const { error: badorderError } = await supabase
        .from("Claims_Badorder")
        .delete()
        .eq("code_pwp", coverCode);
      if (badorderError)
        throw new Error(`Failed to delete Claims_Badorder records: ${badorderError.message}`);
    ...
    // ✅ REGULAR PWP DELETE FLOW
    else if (row.cover_code && row.cover_code.startsWith("C")) {
      // ✅ Fix: Use fallbacks to always get a valid regularCode
      const regularCode =
        row.regularpwpcode || row.regularcode || row.regular_code || row.cover_code;
      ...
      if (approvalError)
        throw new Error(`Failed to delete approval history: ${approvalError.message}`);
      ...
      if (skuError)
        throw new Error(`Failed to delete regular_sku: ${skuError.message}`);
      ...
      if (amountError)
        throw new Error(`Failed to delete amount budget: ${amountError.message}`);
      ...
      const { error: approvedHistoryError } = await supabase
        ...
        .eq("pwp_code", regularCode);
      if (approvedHistoryError)
        throw new Error(`Failed to delete approved history: ${approvedHistoryError.message}`);
      ...
      if (badorderError)
        throw new Error(`Failed to delete regular_badorder: ${badorderError.message}`);
