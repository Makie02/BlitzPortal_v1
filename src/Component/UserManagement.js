...
const UserManagement = ({ setCurrentView }) => {
  ...
  const handleSaveBudgetPermissions = async () => {
    if (!selectedUser?.id) {
    ...
    try {
      // Update only the boolean fields in Account_Users
      ...
      await Swal.fire({
        icon: 'success',
        title: 'Success!',
        text: 'Marketing permissions saved successfully',
        timer: 2000,
      ...
      console.error('Error updating marketing permissions:', error);
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.message || 'Failed to update marketing permissions',
      });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (selectedUser) {
      // assuming selectedUser.budgetpermission = 1 or 2 (or 3 if both)
      if (selectedUser.budgetpermission === 1) {
        setCreateBudget(true);
        setViewBudget(false);
      } else if (selectedUser.budgetpermission === 2) {
        setCreateBudget(false);
        setViewBudget(true);
      } else if (selectedUser.budgetpermission === 3) {
        setCreateBudget(true);
        setViewBudget(true);
      } else {
        setCreateBudget(false);
  ...
  const [pre_regular, setPreRegular] = useState(false);
  ...
  const openModal = async (user, type) => {
    ...
    try {
      // Step 1: Get Supabase user via email or username
      ...
      if (type === 'CreateMarketPermissions') {
        ...
        setRegular(userFromSupabase.Regular === true);
...
const openCreateModal = async () => {
  ...
  const fetchSavedDistributors = async (username) => {
    ...
    if (error) {
      ...
      console.log('Fetched saved distributors:', data.map(d => d.distributor_name));
      setSavedDistributors(data || []);

      // Auto-select saved distributors in checkbox
      const savedNames = (data || []).map(d => d.distributor_name);
      setSelectedDistributors(savedNames);
      // Only set stable copy if not already set
      setStableDistributors(prev => (prev.length === 0 ? data || [] : prev));
    }
  };
  ...
  const handleDeleteDistributor = async (distributorId) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: 'Do you want to delete this distributor?',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Yes, delete it',
    ...
    if (result.isConfirmed) {
      try {
        ...
        // Remove from savedDistributors state
        setSavedDistributors(prev => prev.filter(d => d.id !== distributorId));

        // Remove from selectedDistributors if checked
        const deletedDist = savedDistributors.find(d => d.id === distributorId);
        if (deletedDist) {
          setSelectedDistributors(prev => prev.filter(name => name !== deletedDist.distributor_name));
        }

        Swal.fire('Deleted!', 'Distributor has been removed.', 'success');
      } catch (err) {
  ...
  useEffect(() => {
    if (modalType === 'Distributor' && supabaseUsername) {
      console.log('Fetching distributors for supabaseUsername:', supabaseUsername);
      fetchSavedDistributors(supabaseUsername);
    }
  ...
  const handleSaveDistributors = async () => {
    ...
    // Get selected distributors with their data
    const selected = distributors.filter((d) =>
      selectedDistributors.includes(d.name)
    );

    // Get already saved distributor names
    const alreadySavedNames = savedDistributors.map(sd => sd.distributor_name);

    // Filter out duplicates - only save new ones
    const newDistributorsToSave = selected.filter(dist =>
      !alreadySavedNames.includes(dist.name)
    );

    if (newDistributorsToSave.length === 0) {
      Swal.fire({
        icon: 'info',
        title: 'Already Saved',
        text: 'All selected distributors are already saved.',
      });
      return;
    }

    // Prepare insert records
    const inserts = newDistributorsToSave.map((dist) => ({
      code: dist.code,
    ...
    if (error) {
      ...
      // Don't clear selectedDistributors here - let fetchSavedDistributors handle it
    }
  };
  ...
  const [selectedPermissionCode, setSelectedPermissionCode] = useState(null);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (modalType === "ReferencePermission") {
      fetchPermissions();
    }
  }, [modalType]);
  ...
  async function handleSave() {
    if (!selectedPermissionCode || !selectedUser?.id) return;

    setSaving(true);

    // Save only the code (as a number) into "ReferencePermission"
    const { error } = await supabase
      .from('Account_Users')
      .update({
        ReferencePermission: selectedPermissionCode, // save code here
      })
      .eq('id', selectedUser.id);

    if (error) {
      console.error('Error saving user ReferencePermission:', error.message);
    } else {
    ...
    <Container fluid className="py-4">
      ...
      <div className="mb-4">
        <div className="d-flex flex-column flex-lg-row justify-content-between align-items-start align-items-lg-center gap-3">
          ...
          {/* Filters Form */}
          <Form className="d-flex flex-wrap gap-3 flex-grow-1">
          ...
          <div className="d-flex flex-wrap gap-2">
            ...
            <Button variant="primary" onClick={() => setCurrentView('PermissionsManager')}>
              <FaPlus className="me-2" /> References Permission
            </Button>
          </div>
        </div>
      </div>
                  ...
                  <Card.Body>
                    ...
                    <div className="d-flex justify-content-between flex-wrap mt-3">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => openModal(user, 'CreateMarketPermissions')}
                      ...
                      <Button
                        variant="outline-info"
                        size="sm"
                        onClick={() => openModal(user, 'ReferencePermission')}
        ...
        <Form onSubmit={saveEditedUser}>
          <Modal.Body>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group className="mb-3" controlId="newUserPermissionRole">
                  <Form.Label>Permission Role</Form.Label>
      ...
      >
        ...
        >
          <Modal.Title style={{ fontSize: '20px', fontWeight: '600' }}>
            Manage Distributors for {supabaseUsername}
          </Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body style={{ padding: '24px', backgroundColor: '#f8f9fa' }}>
          {distributors.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 20px', color: '#6c757d' }}>
              <p>No Sales Distributors found.</p>
            </div>
          ) : (
            <>
              {/* Search Bar */}
              <div style={{ marginBottom: '20px' }}>
                <FormControl
                  ...
                  onFocus={(e) => e.target.style.borderColor = '#5b8fa3'}
              ...
              >
                {filteredDistributors.length === 0 ? (
                  ...
                  filteredDistributors.map((dist, idx) => {
                    const isChecked = selectedDistributors.includes(dist.name);
      ...
      <Modal show={modalType === "CreateMarketPermissions" ? true : undefined} onHide={() => setModalType(null)} centered>
        <Modal.Header closeButton>
          <Modal.Title>CREATE MARKETING PERMISSION {selectedUser?.name}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group
            controlId="createBudget"
            className="mt-3 d-flex align-items-center justify-content-between"
          >
            <Form.Label className="mb-0">Create Budget</Form.Label>
            <Form.Check
              type="switch"
              id="create-budget-switch"
              checked={createBudget}
              onChange={() => setCreateBudget(!createBudget)}
            />
          </Form.Group>
          ...
          <Form.Group
            ...
            className="mt-3 d-flex align-items-center justify-content-between"
          >
            <Form.Label className="mb-0">View Budget</Form.Label>
            <Form.Check
              type="switch"
              id="view-budget-switch"
              checked={viewBudget}
              onChange={() => setViewBudget(!viewBudget)}
            />
          </Form.Group>
          <Form.Group
            controlId="regular"
            className="mt-3 d-flex align-items-center justify-content-between"
          >
            <Form.Label className="mb-0">Regular</Form.Label>
            <Form.Check
              type="switch"
              id="regular-switch"
              checked={regular}
          ...
          <Form.Group
            controlId="preRegular"
            className="mt-3 d-flex align-items-center justify-content-between"
          >
            <Form.Label className="mb-0">Pre-Regular</Form.Label>
            <Form.Check
              type="switch"
              id="pre-regular-switch"
              checked={pre_regular}
              onChange={() => setPreRegular(!pre_regular)}
            />
          </Form.Group>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleSaveBudgetPermissions}>
            Save
          </Button>
        </Modal.Footer>
      ...
      <Modal
        show={modalType === "ReferencePermission"}
        onHide={() => setModalType(null)}
        centered
      >
        <Modal.Header closeButton className="bg-primary text-white">
          <Modal.Title>REFERENCE PERMISSION {selectedUser?.name}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group controlId="permissionSelect">
            ...
            <Form.Select
              ...
              onChange={(e) => setSelectedPermissionCode(Number(e.target.value))}
              disabled={saving}
            >
              <option value="" disabled>Select permission</option>
              {permissions.map((perm) => (
                <option key={perm.code} value={perm.code}>
                  {perm.code} - {perm.name || 'Unnamed'}
                </option>
              ))}
            </Form.Select>
          </Form.Group>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleSave} disabled={saving || !selectedPermissionCode}>
            {saving ? 'Saving...' : 'Save'}
          </Button>
        </Modal.Footer>
      </Modal>
