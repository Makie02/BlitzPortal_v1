...
const generateNextCode = () => {
  ...
  const nums = records.map(r => parseInt(r.code, 10) || 0);
  const maxNum = Math.max(...nums);

  // Pad with leading zeros to always be 3 digits
  return String(maxNum + 1).padStart(3, "0");
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            const { error } = await supabase.from('customer_group1').delete().eq('id', id);
            if (error) {
