...
const CustomerGroup2 = () => {
    const [records, setRecords] = useState([]);
    const [loading, setLoading] = useState(true);
    const [modalOpen, setModalOpen] = useState(false);
    const [form, setForm] = useState({ id: null, code: '', name: '', status: true });
    ...
    const fetchRecords = async () => {
        ...
        if (error) {
            Swal.fire('Error', 'Error fetching records: ' + error.message, 'error');
        } else {
            setRecords(data);
        }
        setLoading(false);
    ...
    // Auto-generate CODE starting at 1
    // Auto-generate CODE with prefix KAS
const generateNextCode = () => {
  if (records.length === 0) return "2001";

  const nums = records.map(r => parseInt(r.code, 10) || 2000);
  ...
  return `${maxNum + 1}`;
    ...
    const openAddModal = () => {
        setForm({ id: null, code: generateNextCode(), name: '', status: true });
        setIsEditing(false);
        setModalOpen(true);
    };

    const openEditModal = (rec) => {
        setForm({ id: rec.id, code: rec.code, name: rec.name, status: rec.status });
        setIsEditing(true);
        setModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEditing) {
            ...
            const { error } = await supabase
                .from('customer_group2')
                .insert([{ code: form.code, name: form.name, status: form.status }]);
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            const { error } = await supabase.from('customer_group2').delete().eq('id', id);
            if (error) {
    ...
    const filteredRecords = records.filter(r => {
        ...
        return (
            ...
            (r.status ? 'true' : 'false').includes(term)
        );
    });
                        ...
                        <table style={tableStyle}>
                            ...
                            <tbody>
                                {currentItems.length === 0 ? (
                                    <tr>
                                        <td colSpan="5" style={{ padding: 12, textAlign: 'center' }}>No records found.</td>
                                    </tr>
                                ) : currentItems.map(r => (
                                    <tr key={r.id} style={{ borderBottom: '1px solid #ddd' }}>
                                        ...
                                        <td style={tdStyle}>
                                            <button onClick={() => openEditModal(r)} style={actionBtnStyle}>Edit</button>
                                            <button onClick={() => handleDelete(r.id)} style={{ ...actionBtnStyle, backgroundColor: '#dc3545' }}>Delete</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
