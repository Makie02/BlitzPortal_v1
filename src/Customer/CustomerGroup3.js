...
const CustomerGroup3 = () => {
    ...
    // Auto-generate CODE with prefix KAS3xxx
// Auto-generate CODE with prefix KAS3xxx
// Auto-generate numeric CODE starting at 3000
const generateNextCode = () => {
  if (records.length === 0) return "3000";

  const nums = records.map(r => parseInt(r.code, 10) || 3000);
  ...
  return String(maxNum + 1);
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            const { error } = await supabase.from('customer_group3').delete().eq('id', id);
            if (error) {
