...
// Auto-generate CODE with prefix KAS4000+
const generateNextCode = (records) => {
  if (!records || records.length === 0) return "4000";

  const nums = records.map(r => {
    const match = r.code?.match(/^KAS(\d+)$/);
    return match ? parseInt(match[1], 10) : parseInt(r.code, 10) || 4000;
  });
...
const CustomerGroup4 = () => {
  ...
  const fetchRecords = async () => {
    const { data, error } = await supabase.from("customer_group4").select("*").order("id", { ascending: true });
    if (error) {
      console.error(error);
    } else {
      setRecords(data);
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      const { error } = await supabase.from("customer_group4").update(form).eq("id", editingId);
      if (error) {
      ...
      const { error } = await supabase.from("customer_group4").insert([{ ...form, code: newCode }]);
      if (error) {
        Swal.fire("Error", error.message, "error");
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      const { error } = await supabase.from("customer_group4").delete().eq("id", id);
      if (error) {
        Swal.fire("Error", error.message, "error");
      } else {
